...
| --- | --- | --- | --- | --- | --- |
| `unicom` | 中国联通云盘 | 主写候选 / 同步候选 | `https://panservice.mail.wo.cn` | 手工 token / cookie 注入 | 已打通桌面站 `QueryAllFiles` 认证、对象列举与真实下载；已探测 personal scope，并可通过 `familyId` 或 `QueryFamilyGroups` 自动发现 family scope；浏览器流程配置已覆盖短信登录、个人空间上传、目录创建和删除；Rust 写入路径待补 |
