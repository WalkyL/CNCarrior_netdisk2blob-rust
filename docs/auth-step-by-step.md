...
- 如果浏览器已经在启动中，脚本会等待它完成，而不是再开一个实例
...
13. 如果你要使用 `family` 容器，或者希望保留签名下载兜底，需要继续抓 `AccessToken`。可以在 Network 或页面脚本里搜:

- `AccessToken`
- `accessToken`
- `getAccessToken`

如果你确实拿到了稳定的 `AccessToken`，把它额外保存:
...
如果你只接个人云，且不启用 `family` 容器，也可以先不配 `CCBG_TELECOM_TOKEN_FILE`。`family` 列举、下载和删除需要 `Access Token` 与 `Family ID` 同时可用。
...
CCBG_TELECOM_FAMILY_ID=replace-with-optional-telecom-family-id
```
...
- 中国移动超大文件上传目前仍要视你的账号、上游权限以及宿主机本地 spool/磁盘边界而定，不能默认认为已经和 AList 一样放开
