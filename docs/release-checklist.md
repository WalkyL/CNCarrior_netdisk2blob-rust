...
- [x] 所有 release 编译入口收敛到 `.52`
- [x] GitHub 默认只负责分支、tag、可选 Release 页面和发布记录；当前 self-hosted build-runner workflow 只作为受控 fallback 产出 LXC 与 macOS 资产
- [x] Linux / OpenWrt / Windows / 容器 tar 或镜像二进制默认走 `.52` 或受控局域网 runner
...
- [ ] 如果 `.52` 本地 Linux 构建不可用，已用 GitHub Actions self-hosted build-runner workflow 生成 `ccbg-lxc-package`，并通过 `CCBG_RELEASE_LXC_ASSET_DIR` 合并回 release 目录
...
正式对外 release 必须至少包含下面这些公网安装页会引用的资产。所有资产由 `.52`
...
- [ ] 已登录页面时可直接抓当前会话
- [ ] 上传探针 / 捕获相关面板可正常使用
- [ ] 如果本轮 release 仍没有新的中国移动大文件放行证据，Admin / 文档 / 发布说明必须明确写出“.49 于 2026-07-02 的隔离 `limit-probe` 仍显示 `8 GiB` 被上游 `04010319` 拒绝，`16 GiB` 在当前部署形态下先撞到本地 `No space left on device`”
...
- [ ] provenance 已生成
- [ ] macOS `x86_64` 与 `arm64` 社区/实验包已由 GitHub Actions self-hosted build-runner workflow 生成，并已下载合并回 `.52` release 目录
- [ ] 回滚包 / 上一个稳定版本可用
- [ ] 发布记录已落文档
- [ ] Windows/macOS 后台常驻路径至少经过对应真机 smoke
- [ ] 中国移动能力文案与本轮最新实测一致；没有新证据时，不把 `8 GiB` 的 `04010319` 上游限制或 `16 GiB` 的本地 spool/disk 边界写成“已解决”
...
- GitHub Actions 不再承担通用 CI、自动 Linux/Windows/OpenWrt 发版或 Cloudflare 部署
