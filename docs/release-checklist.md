...
- [ ] 如果 `.47` 本地 Linux 构建不可用，已用 GitHub Actions self-hosted build-runner workflow 生成 `ccbg-lxc-package`，并通过 `CCBG_RELEASE_LXC_ASSET_DIR` 合并回 release 目录
...
- 受控 self-hosted build-runner workflow 可在 `.47` 本地 Linux 构建不可用时产出 LXC fallback 包，也可继续产出 macOS 社区/实验包
- Linux/Windows/OpenWrt release gate 与 Cloudflare 部署改为 `.47` 本地脚本；build-runner 产物下载回 `.47` 后进入同一校验和 `/downloads/latest/*` 发布链路
- macOS 暂时降级为社区/实验包
