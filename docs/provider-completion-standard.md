...
- 共享对象动作历史当前只保留最近 12 条，还没有筛选、导出或按 provider/action 聚合能力。
- 缺少真实账号的自动化 E2E 回归。
...
3. 再完成对象动作与网关复制语义。
4. 最后补齐 probe、文档、测试，才允许标记为 `full`。

如果某个 provider 只满足 1-2 步，只能记为 `partial`，不能在文档或控制面里表述成“已完成”。
