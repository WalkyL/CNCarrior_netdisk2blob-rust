...
5. 文档要明确区分“已实现”和“规划中”。
5.1 中国移动大文件能力必须按最新实测来写。若没有更新的放行证据，继续沿用当前已验证事实：`.49` 在 2026-07-02 的隔离 `limit-probe` 中，`8 GiB` 仍被上游 `04010319` 拒绝，而 `16 GiB` 在当前部署形态下会先撞到本地 `No space left on device`。
