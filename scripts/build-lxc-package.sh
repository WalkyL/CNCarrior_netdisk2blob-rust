...
usage() {
  echo "usage: scripts/build-lxc-package.sh [--target <rust-target>] [--binary <linux-gatewayd>] [--skip-build]"
}
...
install -m 0755 "${ROOT_DIR}/deploy/lxc/ccbg-smb-sidecar.py" "${DIST_DIR}/${PACKAGE_NAME}/scripts/ccbg-smb-sidecar.py"
...
sed -i 's/\r$//' \
  ...
  "${DIST_DIR}/${PACKAGE_NAME}/scripts/ccbg-smb-sidecar.py"
