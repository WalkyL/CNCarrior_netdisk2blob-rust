...
if [ "$(uname -s)" != "Linux" ]; then
  echo "test-smb-sidecar-host-recovery.sh must run on Linux" >&2
  exit 1
fi

if [ "$(id -u)" -ne 0 ]; then
  echo "test-smb-sidecar-host-recovery.sh must run as root" >&2
  exit 1
fi
...
cat > "${control_plane_file}" <<'EOF'
{
  "smb_sidecar": {
    ...
    "data_root": "",
    "workgroup": "WORKGROUP",
    "server_string": "CCBG SMB Sidecar Recovery Test",
    "create_mask": "0660",
    "directory_mask": "0770",
    "disable_splice": true,
    "vfs_objects": ["catia"],
...
echo "[2/5] establish baseline degraded runtime in no-fuse/direct-process mode"
...
baseline_pid="$(json_extract "${status_file}" "processes.0.pid")"
baseline_hash="$(json_extract "${metadata_file}" "desired_hash")"

echo "[3/5] corrupt metadata/status to fake a healthy share state with same desired hash"
python3 - "${status_file}" "${metadata_file}" <<'PY'
import json
import sys

status_path, metadata_path = sys.argv[1:3]
for path in (status_path, metadata_path):
    with open(path, "r", encoding="utf-8") as handle:
        payload = json.load(handle)
    payload["state"] = "running"
    payload["listener_ready"] = True
    payload["last_error"] = None
    payload["process_count"] = 2
    payload["mounted_share_count"] = 1
    payload["share_states"] = [
        {
            "id": "root",
            "share_name": "CCBGRoot",
            "mount_path": payload["mount_root"] + "/root",
            "remote_path": "root",
            "read_only": False,
            "pid": 999999,
            "unit_name": "fake-rclone.service",
            "mounted": True,
            "running": True,
            "rss_bytes": 1,
            "log_path": payload["runtime_root"] + "/logs/fake-rclone.log"
        }
    ]
    payload["processes"] = [
        {
            ...
            "running": True,
            "rss_bytes": 1,
            "unit_name": "fake-smbd.service",
            "log_path": payload["runtime_root"] + "/logs/fake-smbd.log"
        },
        {
            "role": "rclone:root",
            "pid": 999999,
...
echo "[4/5] rerun sync and verify helper re-reconciles instead of trusting stale metadata"
sleep 1
run_helper sync env CCBG_SMB_SIDECAR_FORCE_NO_SYSTEMD_RUN=1 CCBG_SMB_SIDECAR_FORCE_NO_FUSE=1 >/dev/null
assert_json_field "${status_file}" "state" '"degraded"'
assert_json_field "${status_file}" "mounted_share_count" '0'
assert_json_field "${status_file}" "process_count" '1'
recovered_hash="$(json_extract "${metadata_file}" "desired_hash")"
recovered_pid="$(json_extract "${status_file}" "processes.0.pid")"
if [ "${recovered_hash}" != "${baseline_hash}" ]; then
  echo "expected desired_hash to remain stable across stale-state recovery" >&2
  exit 1
fi
if [ "${recovered_pid}" = "${baseline_pid}" ]; then
  echo "expected stale-state recovery to restart smbd and change pid" >&2
  exit 1
fi
...
echo "[5/5] stop and re-sync converge again from stopped state"
...
echo "smb-sidecar-host recovery checks passed"
