...
impl ProviderBridgeCatalog {
    ...
    pub fn credential_mapping_for(
        &self,
        credential_key: &str,
    ) -> Option<&ProviderBridgeCredentialMapping> {
    ...
    fn validate(&self, source_label: &str) -> Result<(), BlobError> {
        ...
        if let Some(binding) = self.browser_profile.as_ref() {
            for (label, value) in [
                (
                    "source_url_runtime_key",
                    binding.source_url_runtime_key.as_deref(),
                ),
                (
                    "user_agent_runtime_key",
                    binding.user_agent_runtime_key.as_deref(),
                ),
                (
                    "headers_runtime_key",
                    binding.headers_runtime_key.as_deref(),
                ),
            ] {
