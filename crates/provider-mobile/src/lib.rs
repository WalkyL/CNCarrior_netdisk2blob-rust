...
use blob_core::{
    ...
    ObjectBody, ObjectInfo, ObjectPayload, OutboundIpFamily, PutObjectRequest,
    PutObjectResult, RenameObjectRequest, ServiceHealth, SharedBodySpoolObserver,
    StorageCapacity, StorageScopeHealth, StorageScopeKind, TokenSource,
};
...
struct MobileFileCreateData {
    ...
    #[serde(rename = "partInfos", default, deserialize_with = "deserialize_vec_or_null")]
...
impl MobileBlobAdapter {
    ...
    fn base_headers(&self) -> Result<Vec<(HeaderName, HeaderValue)>, BlobError> {
        ...
        headers.push((USER_AGENT, HeaderValue::from_str(self.effective_user_agent()).map_err(|error| {
            BlobError::Configuration(format!("invalid China Mobile user agent: {error}"))
        })?));
        headers.push((ACCEPT, HeaderValue::from_static("application/json, text/plain, */*")));
        headers.push((CONTENT_TYPE, HeaderValue::from_static("application/json;charset=UTF-8")));
        headers.push((
            AUTHORIZATION,
    ...
    async fn prepare_upload_body(
        ...
        Ok(PreparedMobileUpload {
            ...
            part_size_bytes: self.effective_upload_part_size_bytes(total_len),
    ...
    async fn upload_mobile_parts(
        ...
        for (index, part) in plan.part_infos.iter().enumerate() {
            ...
            let response = self
                ...
                .send()
                .await
                .map_err(|error| BlobError::Upstream(format!("China Mobile part upload failed: {error}")))?;
    ...
    async fn complete_mobile_upload(
        ...
        let upload_id = plan.upload_id.clone().ok_or_else(|| {
            BlobError::Upstream(
                "China Mobile file/complete requires an uploadId for non-rapid uploads"
                    .to_string(),
            )
        })?;
        let mut body = self
    ...
    async fn ensure_folder(
        ...
        if let Some(existing) = self.find_child_folder_id(parent_file_id, folder_name).await? {
            return Ok(existing);
        }

        let created = self.create_mobile_folder(folder_name, parent_file_id).await?;
        Ok(created.file_id)
    }
    ...
    async fn resolve_container_root_folder_id(
        ...
        self.resolve_directory_path_if_exists(managed_root.as_str()).await
    }
    ...
    async fn resolve_object_entry(
        ...
        let Some(container_root_id) = self.resolve_container_root_folder_id(container).await? else {
        ...
        loop {
            let page = self
                ...
                .await?;
            if let Some(entry) = page.items.into_iter().find(|item| {
                !item.is_folder() && item.display_name() == Some(file_name)
            }) {
...
impl BlobBackend for MobileBlobAdapter {
    ...
    async fn put_object(&self, request: PutObjectRequest) -> Result<PutObjectResult, BlobError> {
        ...
        let plan = self
            ...
            .await?;
        let first_response_latency_ms = elapsed_millis(create_started_at);
        if plan.rapid_upload {
        ...
        self.upload_mobile_parts(&upload, &plan).await?;
    ...
    async fn delete_object(&self, container: &str, key: &str) -> Result<(), BlobError> {
        let object_path = self.managed_object_key(container, key)?;
        Err(BlobError::NotImplemented(format!(
            "China Mobile native delete is not completed yet; managed object path resolved to {object_path}"
        )))
    }

    async fn rename_object(&self, request: RenameObjectRequest) -> Result<(), BlobError> {
        let source = self.managed_object_key(&request.container, &request.key)?;
        let destination = self.managed_object_key(&request.container, &request.new_key)?;
        Err(BlobError::NotImplemented(format!(
            "China Mobile native rename is not completed yet; source={source} destination={destination}"
        )))
    }
    ...
    async fn move_object(&self, request: MoveObjectRequest) -> Result<(), BlobError> {
        ...
        let destination =
            self.managed_object_key(&request.destination_container, &request.destination_key)?;
        Err(BlobError::NotImplemented(format!(
            "China Mobile native move is not completed yet; source={source} destination={destination}"
        )))
    }
}
...
fn build_http_client(config: &MobileConfig) -> Result<reqwest::Client, BlobError> {
    let mut builder =
        reqwest::Client::builder().connect_timeout(Duration::from_secs(config.request_timeout_secs.max(1)));
...
mod tests {
    ...
    fn rejects_duplicate_capability_ids() {
        ...
        let error = load_mobile_native_capabilities(temp.path().to_str()).expect_err("duplicate id should fail");
        assert!(error.to_string().contains("duplicate China Mobile native capability id"));
    }

    #[test]
    fn capability_catalog_fields_are_available_for_runtime_execution() {
        let capabilities = load_mobile_native_capabilities(None).expect("default capabilities");
        let create = capabilities.get("file_create").expect("file_create capability");
        ...
        assert_eq!(download.signature_strategy.as_deref(), Some("mcloud_md5_v1"));
    }
    ...
    async fn managed_root_head_list_and_get_are_scoped_under_one_provider_folder() {
        ...
        config.native_capability_catalog_path = Some(
            catalog
                .path()
                .to_str()
                .expect("catalog path")
        ...
        assert_eq!(nested.key, "ccbg-verification/diag-unicom-nested-20260520.txt");
    ...
    async fn put_object_streams_to_spool_and_uploads_mobile_parts() {
        ...
        let address = listener.local_addr().expect("mock mobile upload local addr");
        let base_url = format!("http://{address}");
