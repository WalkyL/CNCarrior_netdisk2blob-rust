...
use blob_core::{
    ...
    ServiceHealth, StorageCapacity, StorageScopeHealth, StorageScopeKind,
    StreamFirstProgressObserver, TokenSource,
};
...
impl UnicomBlobAdapter {
    ...
    async fn upload_chunk(
        ...
        for attempt in 1..=DEFAULT_UNICOM_UPLOAD_CHUNK_MAX_ATTEMPTS {
            ...
            let file_part = Part::stream_with_length(
                reqwest::Body::wrap_stream(futures_util::stream::once({
            ...
            match parse_upload_chunk_response(response, &action).await {
                Ok(result) => {
                    return Ok(TimedUploadChunkResponse {
                        response: result,
                        first_response_latency_ms: match first_upload_progress_ms
                            .load(Ordering::SeqCst)
                        {
                            0 => elapsed_millis(upload_started_at).max(1),
                            value => value,
                        },
                    });
                }
...
impl BlobBackend for UnicomBlobAdapter {
    ...
    fn capabilities(&self) -> BackendCapabilities {
        BackendCapabilities {
            ...
            upload_part_size_bytes: Some(match self.config.upload_chunk_size_bytes {
                0 => DEFAULT_UNICOM_UPLOAD_CHUNK_SIZE_BYTES,
                value => value,
            }),
        }
    }
    ...
    async fn put_object(&self, request: PutObjectRequest) -> Result<PutObjectResult, BlobError> {
        ...
        let chunk_size = request
            ...
            .unwrap_or(self.upload_chunk_size_bytes()?);
    ...
    async fn move_object(&self, request: MoveObjectRequest) -> Result<(), BlobError> {
        ...
        .await?;
        if entry.display_name() != Some(target_name.as_str()) {
            self.rename_object_entry(&destination_scope, &entry, &target_name)
                .await?;
        }
        Ok(())
    }
...
mod tests {
    ...
    async fn managed_root_maps_user_keys_under_single_provider_folder() {
        let token = "341e39ff-91a6-4a86-9a98-6cd41501b2a8";
        let server = MockServer::start(sample_entries(), sample_file_bodies(), token).await;
        let adapter = mock_unicom_adapter_with_root_prefix(&server.base_url, token, "ccbg-managed");
        ...
        assert!(
            !listed
                .iter()
                .any(|object| object.key.contains("ccbg-managed"))
        );
    ...
    async fn move_object_uses_native_move_dispatcher_with_family_source_hint() {
        ...
        let rename_request = requests
            .iter()
            .find(|request| request["__operation"] == RENAME_FILE_OR_DIRECTORY_OPERATION)
            .expect("RenameFileOrDirectory request should be recorded after move filename change");
        assert_eq!(rename_request["spaceType"], "0");
        assert_eq!(rename_request["type"], 1);
        assert_eq!(rename_request["id"], "family-file");
        assert_eq!(rename_request["name"], "moved-family-note.txt");
    }
}
