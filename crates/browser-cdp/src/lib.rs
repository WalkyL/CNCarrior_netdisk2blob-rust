...
const SET_INPUT_FUNCTION_SOURCE: &str = r#"function(value, eventNames) {
    const element = this;
    ...
    if (typeof element.setSelectionRange === 'function') {
        try {
            element.setSelectionRange(normalizedValue.length, normalizedValue.length);
        } catch (_) {}
    }
    ...
    for (const eventName of eventNames) {
        ...
        if (eventName === 'input') {
            ...
            const key = normalizedValue.slice(-1) || '0';
            ...
            event = new Event(eventName, { bubbles: true, composed: true });
        }
...
const DISPATCH_EVENTS_FUNCTION_SOURCE: &str = r#"function(eventNames) {
    for (const eventName of eventNames) {
        if (!eventName) {
            continue;
        }
        ...
        if (eventName === 'input') {
            if (typeof InputEvent === 'function') {
                event = new InputEvent('input', { bubbles: true, composed: true });
            } else {
                event = new Event('input', { bubbles: true, composed: true });
            }
        } else if (eventName === 'keyup' || eventName === 'keydown' || eventName === 'keypress') {
...
impl BrowserFlowSession for CdpBrowserFlowSession {
    ...
    async fn dispatch_events(
        ...
        let result = self
            .call_function_on_object(
                &object_id,
                DISPATCH_EVENTS_FUNCTION_SOURCE,
                &[Value::Array(
                    events.iter().cloned().map(Value::String).collect(),
                )],
            )
            .await;
        self.release_object(&object_id).await;
...
mod tests {
    ...
    #[test]
    fn set_input_uses_native_setter_and_framework_friendly_events() {
        assert!(SET_INPUT_FUNCTION_SOURCE.contains("Object.getOwnPropertyDescriptor"));
        assert!(SET_INPUT_FUNCTION_SOURCE.contains("descriptor.set.call"));
        assert!(SET_INPUT_FUNCTION_SOURCE.contains("InputEvent"));
        assert!(SET_INPUT_FUNCTION_SOURCE.contains("KeyboardEvent"));
        assert!(SET_INPUT_FUNCTION_SOURCE.contains("_valueTracker"));
    }

    #[test]
    fn dispatch_events_supports_input_and_keyboard_events() {
        assert!(DISPATCH_EVENTS_FUNCTION_SOURCE.contains("InputEvent"));
        assert!(DISPATCH_EVENTS_FUNCTION_SOURCE.contains("KeyboardEvent"));
        assert!(DISPATCH_EVENTS_FUNCTION_SOURCE.contains("composed: true"));
    }
